...
<div class="page-header">
    <div class="page-header-content">
        <div class="page-title">
            <h1><i class="fas fa-tags text-gold me-2"></i> تصنيفات المصروفات</h1>
            <p class="text-muted">إدارة تصنيفات المصروفات</p>
        </div>
    </div>
</div>
                            ...
                            <tbody>
                                @foreach($categories as $category)
                                    <tr>
                                        ...
                                        <td><span class="badge-count">{{ $category->expenses_count }}</span></td>
                                        <td>
                                        ...
                                        <td>
                                            ...
                                            <form method="POST" action="{{ route('expenses.categories.destroy', $category->id) }}" class="d-inline" onsubmit="return confirm('هل أنت متأكد من حذف هذا التصنيف؟')">
                                                @csrf @method('DELETE')
                ...
                <form method="POST" action="{{ route('expenses.categories.update', $category->id) }}">
                    ...
                    <div class="modal-body">
                        ...
                        <div class="form-check form-switch">
                            <input type="checkbox" name="is_active" class="form-check-input" value="1" id="active{{ $category->id }}" {{ $category->is_active ? 'checked' : '' }}>
                            <label class="form-check-label" for="active{{ $category->id }}">نشط</label>
                        </div>
                    </div>
